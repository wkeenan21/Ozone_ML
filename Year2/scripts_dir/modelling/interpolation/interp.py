...
def normalize_givenMandR(df, cols, rdict, mdict):
    df2 = df.copy()
    for col in cols:
        df2[col] = (df2[col] - mdict[col]) / rdict[col]
    return df2
...
def multistep_forecast_noO3(model, hours_ahead, input_ia, input_da, input_dates, ozone_column=None):
    """
    Perform multistep forecasting using an LSTM model.
    Parameters:
    - model: Keras model object
    ...
    for i in range(hours_ahead):
        # test for time continuity. Some arrays will not be time continuous because of missing data. Remove them.
        bad_times = time_continuity_test(input_sequence, 9, 10) # this assumes the day col and the hour col are the 9 and 10 columns
        # remove bad times
        input_sequence = input_sequence[bad_times]
        nextDA = nextDA[bad_times]
        next_dates = next_dates[bad_times]
...
def split_data(O3Jn, sets, cols, one_hot, timesize, sitename='site_name', for_grid=False):
    trainIAs = {}
    trainDAs = {}
    trainDates = {}
    tIAs = {}
    ...
    if one_hot:
        cols = cols + list(O3Jn[sitename].unique())

    prepped_data = []
    for s in sets:
        print(f'set {count}')
        dfs = dict(tuple(s.groupby(sitename)))
        df_list.append(dfs)
        count+=1
...
'''get the grid data, do all the same pre-processing'''
grid = r"D:\Will_Git\Ozone_ML\Year2\Merged_Data\10kgrid_merge.csv"
O3Jgrid = pd.read_csv(grid)

# remove columns
remove = []
for col in O3Jgrid.columns:
    if 'Unnamed' in col or 'pressure' in col:
        remove.append(col)
O3Jgrid = O3Jgrid.drop(columns=remove)
# rename ozone
O3Jgrid.rename(columns={'sample_measurement':'o3'}, inplace=True)
# make columns for day and hour of day
O3Jgrid['datetime'] = pd.to_datetime(O3Jgrid['datetime'], utc=False)

# remove values that are zero (never 0 ozone in the atmosphere). They will be interpolated later
O3Jgrid['o3'].where(O3Jgrid['o3'] > 0, other=np.nan, inplace=True)
#fifthP = O3Jgrid['o3'].quantile(q=0.05)
# fill missing hours
dfs = dict(tuple(O3Jgrid.groupby('site_name')))
new_dfs = []
# decide your timesize
timesize = 24
# give it a more intuitive site naming
...
O3Jgrid2 = pd.concat(new_dfs, ignore_index=True)
# normalize
O3Jgrid2n, rdict, mdict = normalize(O3Jgrid2, cols)
# normalize the site data based on the stats of the bigger dataset
O3Jn = normalize_givenMandR(O3J, cols, rdict, mdict)
...
# prep the grid data
gridIAs, gridDAs, gridDates = split_data(O3Jgrid2n, [O3Jgrid2n], cols, False, 24, 'coords_x', True)

# stack them into big arrays for training the universal model. We really only need training
...
#vIA_noO3 = np.delete(, 0, axis=2)


#model = runLSTM(trainIA, trainDA, 72, cols=cols, activation='sigmoid', epochs=25, units=32, run_model=True)
model_one_hot = trainLSTMgpt(trainIA_f_24, trainDA_f_24, epochs=50, layers=3)
...
for site in gridIAs.keys():
    ...
    lat, lon = site.split(',')
    name = str(round(float(lat), 4)) + '_' + str(round(float(lon), 4))
    st_vIA = gridIAs[site].copy()
    st_vDA = gridDAs[site].copy()
    st_vDates = gridDates[site].copy()
    ...
    metrics = {}
    metrics['site'] = site
    preds, actuals, dates, rms_d, rsq_d = multistep_forecast(model_one_hot, 8, st_vIA, st_vDA, st_vDates, ozone_column=0)
    for i in rms_d.keys():
        metrics[i] = rms_d[i]
    for j in rsq_d.keys():
        metrics[j] = rsq_d[j]
    ...
    merged_df.to_csv(r"D:\Will_Git\Ozone_ML\Year2\results\grid_results\{}.csv".format(name))
