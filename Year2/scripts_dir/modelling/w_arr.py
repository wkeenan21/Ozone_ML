...
def dataPrep(df, timesize, cols, dep_var, w_w=False):
    """
    ...
    colsnoO = list(filter(lambda x: x != dep_var, cols)) # get all the cols minus dep_var
    w_arr = np.zeros(shape=((len(df)), 1, len(colsnoO)))
    ind_arr[:] = np.nan
    dep_arr[:] = np.nan
    ...
    for index, row in df.iterrows():
        if index < (len(df) - timesize):
            ...
            w = df[colsnoO][index + timesize:index+timesize+1]
            w_arr[index] = w
    ...
    if not w_w:
        ...
        return ind_arr, dep_arr, dateArray, w_arr
...
def evaluate(actual, pred):
    ...
    print(f'actual mean: {np.mean(actual)}, pred mean: {np.mean(pred)}')
...
#         # evaluate
#         rms, rsq = evaluate(daUn, predictionsUn)
...
def multistep_forecast_noO3(model, hours_ahead, input_ia, input_da, input_dates, ozone_column=None, day_col=9,hour_col=10):
    ...
    for i in range(hours_ahead):
        # test for time continuity. Some arrays will not be time continuous because of missing data. Remove them.
        input_sequence, bad_times = time_test(input_sequence, day_col,
                                              hour_col)  # this assumes the day col and the hour col are the 9 and 10 columns
        # remove bad times
        # input_sequence = input_sequence[bad_times]
        nextDA = nextDA[bad_times]
        next_dates = next_dates[bad_times]
        ...
        # unNormalize them so you can evaluate
        predictionsUn = unNormalize(predicted_step, rdict['o3'], mdict['o3']) * 1000
        daUn = unNormalize(nextDA, rdict['o3'], mdict['o3']) * 1000
        # evaluate
        rms, rsq = evaluate(daUn, predictionsUn)
        metrics_rms[f'rms{i}'] = rms
        metrics_rsq[f'rsq{i}'] = rsq
...
def split_data(O3Jn, sets, cols, one_hot, timesize, w_w=False):
    trainIAs = {}
    trainDAs = {}
    trainDates = {}
    trainW = {}
    ...
    for s in sets:
        ...
        for df in dfs.values():
            ...
            if not w_w:
                ia, da, dates = dataPrep(df, timesize, cols, 'o3', w_w=w_w)
            elif w_w:
                ia, da, dates, w_arr = dataPrep(df, timesize, cols, 'o3', w_w=w_w)
            ...
            if count == 1:  # just verifies this is the training data
                ...
                return trainIAs, trainDAs, trainDates, trainW, vIAs, vDAs, vDates, vW, tIAs, tDAs, tDates, tW, df_list
                tIAs[site] = ia
                tDAs[site] = da
                tDates[site] = dates
                tW[site] = w_arr
...
def single_step_modelling(trainIA, trainDA, hours, day_col=10, hour_col=11, epochs=25, units=64, drop=0.2, batch=64,
                          layers=2, loss='mean_absolute_error'):
    # must train a model for each forecast horizon
    models = {}
    # test for time continuity. Some arrays will not be time continuous because of missing data. Remove them.
    trainIA, bad_times = time_test(trainIA, day_col,
                                   hour_col)  # this assumes the day col and the hour col are the 9 and 10 columns
    # remove bad times
...
def single_step_forecasting(models, vIA, vDA, vDates, day_col=10, hour_col=11):
    ...
    rsq_d = {}
    # test for time continuity. Some arrays will not be time continuous because of missing data. Remove them.
    vIA, bad_times = time_test(vIA, day_col=day_col,
                               hour_col=hour_col)  # this assumes the day col and the hour col are the 9 and 10 columns
    # remove bad times
    vDA = vDA[bad_times]
...
for adf in dfs.values():
    df = fill_missing_hours(adf, 'datetime', target_months=[5, 6, 7, 8, 9],
                            constant_columns=['county_code', 'site_number', 'county', 'site', 'site_name', 'pop_den'])
...
trainIAs_t_24, trainDAs_t_24, trainDates_t_24, trainW, vIAs_t_24, vDAs_t_24, vDates_t_24, vW, tIAs_t_24, tDAs_t_24, tDates_t_24, tW, dfs2 = split_data(O3Jn, sets, cols, True, 24, w_w=True)
...
trainWstack = np.vstack(list(trainW.values()))

model_one_hot = trainLSTMgpt(trainIA_t_24, trainDA_t_24, epochs=10, layers=3, loss='mean_absolute_error')


site='Boulder'
preds, actuals, dates, rms_d, rsq_d = multistep_forecast_noO3(model_one_hot, 6, vIAs_t_24[site], vDAs_t_24[site], vDates_t_24[site], ozone_column=0, day_col=10, hour_col=11)

newIA = np.concatenate([trainWstack, preds], axis=0)
model2 = trainLSTMgpt(newIA, trainDA_t_24, epochs=10)